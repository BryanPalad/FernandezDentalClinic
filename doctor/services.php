...
?>
 <?php 
        if(isset($_POST['update']))
        {
          ...
          $update = mysqli_query($con,"UPDATE service set service_type='$servicetype',service_name='$servicename',service_time='$servicetime' where service_id=$serviceid");
          ?>
                        ...
                        <table class="table table-hover table-bordered">
                            ...
                            while ($services=mysqli_fetch_array($result)) {
                                ...
                                echo "<tr>";
                                    ...
                                    <a style='margin: 2px' href='updateservices.php?serviceid=".$services['service_id']."' id='".$services['service_id']."' name='modify' class='Edit' title='Edit Service'><img src='assets/img/edit_16.png' aria-hidden='true'></a>
                                    <a href='#modalservice' id='".$services['service_id']."' class='delete'><img src='assets/img/trash_16.png' aria-hidden='true' title='Remove service'></span></a>
            ...
            <div class="modal fade" id="modalservice" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <select name="service_type" class="form-control input-lg" required="">
                               <option></option> 

                              <?php

                              $select = mysqli_query($con,"SELECT * from service_type");
                              while ($selected = mysqli_fetch_array($select)) {

                             ?>
                             <option value="<?php echo $selected['service_type']; ?>"><?php echo $selected['service_type']; ?></option>
                              <?php }
                                    ?>  </select>
                                <br>
                                  ...
                                  <select name="time" class="form-control input-lg" required="">
                                      ...
                                      <option></option>
                                                    ...
                                                    <option value="75">1 hour and above</option>
                                  </select>
                                  <br>
                                  <div class="form-group">
                                    ...
                                    <form action="<?php $_PHP_SELF ?>" method="POST" accept-charset="utf-8" class="form" role="form" onsubmit="return('Are you sure you want to add service type?');">
                                         <input type="text" name="newtype" class="form-control input-lg" required="">
                                         <br>
                                    <input type="submit" name="addnew" class="btn btn-primary" value="Add Service Type" >
                                            </div>
...
{
    $servicetype = $_POST['newtype'];

    $selecttype = mysqli_query($con,"SELECT * from service_type where service_type ='$servicetype'");
    $selected = mysqli_fetch_array($selecttype);
    if($selected['servicetype'] ==1)
    {
        ?>
        <script type="text/javascript">
            alert('Service Type already exists, Please Try again');
        </script>
        <?php
    }
    else
    {
  $query = "INSERT INTO service_type (id,service_type)
        VALUES (default,'$servicetype')";
    }
        $result = mysqli_query($con, $query);
// echo $result;
if( $result )
...
?>
<script type="text/javascript">
alert('Successfully added service type');
